...
# ── 1. Kirim email dari file HTML ─────────────────────────────────────────────
def send_email_from_file():
    with open('/opt/airflow/templates/demo_template.html', 'r') as f:
        html_content = f.read()
    ...
    send_email(
        ...
        cc=[],
        subject='[DEMO] Email dari File HTML',
        html_content=html_content,
    )

# ── DAG ───────────────────────────────────────────────────────────────────────
with DAG(
    dag_id='3c-email-html',
    default_args=default_args,
    description='Demo DAG - 2 jenis pengiriman email',
    ...
    task_email_from_file = PythonOperator(
        task_id='send_email_from_file',
        python_callable=send_email_from_file,
    )
    ...
    start >> task_email_from_file >> end